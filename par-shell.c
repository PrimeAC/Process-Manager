...
sem_t numProcessos, semFilhos;/*semaforos*/
	...
	while (1){
	    ...
	    if (num_filhos==0) {/*nao existem processos filhos*/
	     	...
	     	if(flag==0){/*verifica se foi acionado o comando exit*/
	        	...
	        	sem_wait(&semFilhos);/*bloqueia o processo quando nao existem filhos em execucao*/
	      	}
...
int main(int argc, char* argv[]){
	...
	sem_init(&numProcessos,0,MAXPAR);/*inicializa o semaforo que controla o numero de processos em simultaneo*/
	sem_init(&semFilhos,0,0);/*inicializa o semaforo que verifica a existencia de filhos*/
	...
	while(1){
		...
		if(readLineArguments(argVector, NARGUMENTOS)>0){ /*verifica se o utilizador escreveu algo */
			...
			if(strcmp(argVector[0], "exit")==0){
			    ...
			    sem_post(&semFilhos);/*assinala o desbloqueio da tarefa monitora para que esta termine*/
		  	...
		  	sem_post(&semFilhos);/*assinala que um filho foi criado*/
